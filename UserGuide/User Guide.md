...
![image](images/tools/BlotTool.png)     
...
The **Channels** submenus contain options related to the channels within figure. It will appear inside of the popup menu for [image panels](#Image-Panels) and other objects. Depending on the object clicked and the context, slightly different versions of the channels submenu will appear. The channels menus contain the following items
  ...
  -	**"Invert Channel Panel Color**. Makes the color of the channel panel inverted. This will invert the color for the image panel that was clicked on and others panels that contain the same channel. Will not work if there are multiple channels in the same panel. This option does not affect the merge panel. 
...
The **Figure** submenu appears in the popup menus for layouts and image panels if those objects are part of a figure. It will appear inside of the popup menu for [image panels](#Image-Panels) and [layouts](#Layouts).
 ...
 -	Items in the **Add Labels** submenu create an entire set of labels at once. 
        - **Generate Row/Coloumn/Panels labels** create labels for every single row, columns or panels of the figure. The text of these labels might be derived from the file names of the images used in the figure (user can change this with the **Label Creation Options** menu item). 
        - The **Add Lane Labels** option brings up the [lane label dialog](#Lane-Labels) and creates the labels for each lane of a gel, blots, TLC or other assay. These labels are attached to their own layout which is placed above the image. User can change the distance between labels by editing their layout. 
        - The **Add Band Marks** creates a series of horizontal lines/arrows with attached label text (the 'markers'). These can be used to as molecular weight markers or just as a way to point out specific bands. These are aligned to each other automatically. You can move a band up/down by clicking and dragging. When a band marker is selected, you may click the cyan handles to move/edit the entire group of markers. You can use the mini tool bar below the markers to change the appearance of one or more of them. 
...
- The **Toolbars** menu allows the user to open specific toolbars. Users may close toolbars that they do not need and open them again using this menu. Only the main toolbar is critical for use of QuickFigures, the others are optional.
	...
	![image](images/minitoolbar/SmartToolbar3.png)  
...
### Help Menu
The help menu provides information and tutorials that will help you use QuickFigures
...
### Insets
One can draw a region of interest (ROI) on an image panel and create several additional 'daughter' panels that show that region of interest at a higher level of magnification (see image below) using either the [image popup menu](#Image-Panel-Popup-Menu) or the inset tool located within the [main toolbar](#The-Basics-of-Using-QuickFigures). When that ROI is moved, resized or rotated by the user, the area shown by the daughter panels changes.
This object also has a special popup menu with options that affect the daughter panels. 
That popup menu has few functions.
 ...
 - **Redo Inset Layout** displays a dialog with options to alter the position of the daughter panels. Every time one changes the options in that dialog, the panels are moved. By dragging the mouse near the blue rectangle in the dialog, a user can change the location of the inset panels relative to their parent panel.
 - **Channel Label** submenu contains options that apply to the channel labels attached to the daughter panels. See section on the [Channel Label Submenu](#Channel-Label-Submenu)
  ...
  - **Square Lock On/Off** changes the inset between square lock mode and normal mode. When in square lock mode, the region of interest will maintain a square shape whenever the user drags a handle.
   - **Mirror to sister panels** can be used if the parent panel is part of a series of panels representing split channels. this option crates additional rectangles in the other panels whose location will match the original rectangle for ones insets.
