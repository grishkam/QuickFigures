...
/**during my time programming this package I repeatedly wrote code to do several things on the same field of a given class
 1) add the value of the field to image metadata with a key.
 2) set the value of a field to the value described by image metadata.
 3) add items representing said values to a user interface (normally imageJ's generic dialog ) 
 4) set the values of a field based on changes to the items in the user interface (normally imageJ's generic dialog)
...
public @interface RetrievableOption {
	...
	/**String key for adding values of this field to either a hashtable or the metadata of an image*/
	String key();
	...
	/**Should this item be added to image metadata and retrived?*/
	boolean save() default true;
	/**Whether to match this field when creating a double of the diven object*/
	...
	//Class<?> getPatameterType default Object.class;
	
	/**A number used for sorting fields in a particular order*/
	int rank() default 0;
	
	/**the length if the object is an array*/
