...
/**This is the class that keeps the layout of panels. 
   ...
   */
public class BasicLayout implements LayoutSpaces,GridLayout, Serializable, PanelLayout{
 ...
 public void resetPtsPanels() {
	...
	/**constructs a layout with the given dimensions*/
	 	 public BasicLayout(int cols, int rows, int panelWidth, int panelHeight, int hBorder, int vBorder , boolean rowmajor) {
	 	...
	 	/**sets fields based on the values stored in the given object*/
	 	public BasicLayout(MetaInfoWrapper mdw) {
	 		setToMetaData(mdw);
	 	}
	 	...
	 	private void setToMetaData(MetaInfoWrapper impmw) {
		   ...
		   /**sets all horizontal data to 0 resulting in a layout with 1 column only*/
		   ...
		   /**returns a version of this layout in which that space that
		     ...
		     part of the panel width. does not take into account columns 
		   ...
		   private void convertoPanelRowLayout() {
			...
			public Rectangle2D getContaingPanel( int x, int y) {
			 ...
			 /**Creates an altered duplicate of the layout according to the given type*/
			   public BasicLayout makeAltered(int type) {
				   BasicLayout out=duplicate();
			   ...
			   * see motnage spaces for possible arguments*/
			   ...
			   public Shape getSelectedSpace(int i, int type) {
				   BasicLayout ml=makeAltered(type);
			   ...
			   public Shape getSelectedSpace(int type) {
				   return this.getSelectedSpace(1, type);
			   }
			   
			 private Shape getDifferenceSpace(int i, int type1, int type2) {
			 ...
			 private Shape getSumSpace(int i, int[] types, boolean bound) {
			...
			private  Shape getOverlapSpace(int i, int type1, int type2) {
					 Area r1=new Area(getSelectedSpace(i, type1));
					 Area r2=new Area(getSelectedSpace(i, type2));
					 r2.intersect(r1);
					 return r2;
				}
			private  Shape getAllPanelsSpace(int type) {
			...
			private  Shape getAllPanelsInRowSpace(int row, int type) {
			...
			private  Shape getAllPanelsInIndexRangeSpace(int start, int end, int type) {
				  Area r1=new Area(getSelectedSpace(start, type));
				  for (int i=start+1; i<=end; i++) {r1.add(new Area(getSelectedSpace(i, type)));}
				  return r1;
			  }
			  
			private  Shape getAllPanelsInColSpace(int col, int type) {
			...
			/**does nothing but may be used later*/
			...
			/**returns true if any of the panels have unique widths (not the standard)*/
			...
			/**returns true if any of the panels have unique heights (not the standard)*/
			...
			/**returns true if the panel has a unique width (not the standard)*/
			@Override
			public boolean doesPanelUseUniqueWidth(int panel) {
				int col=this.getColAtIndex(panel);
				return this.columnHasIndividualWidth(col);
			}
			/**returns true if the panel has a unique height (not the standard)*/
			@Override
			...
			/**Called to alter the the size of a given panel based on how a mouse is moving (to location X,Y)*/
			@Override
			public void nudgePanelDimensions(int panelnum, double nudgeX, double nudgeY) {
				int col=this.getColAtIndex(panelnum);
				int row=this.getRowAtIndex(panelnum);
				
				double w;
				if (doesPanelUseUniqueWidth(panelnum)) w = this.getPanel(panelnum).getWidth()+nudgeX;
				else w=this.getPanel(panelnum).getWidth()+nudgeX/col;
				
				double h;

				if (doesPanelUseUniqueWidth(panelnum)) h	= this.getPanel(panelnum).getHeight()+nudgeY;
				else h= this.getPanel(panelnum).getHeight()+nudgeY/row;
			...
			/**returns a list of GridEditListeners that are attached to the layout*/
			@Override
