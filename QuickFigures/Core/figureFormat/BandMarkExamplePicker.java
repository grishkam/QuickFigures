...
public class BandMarkExamplePicker extends GraphicalItemPicker<ArrowGraphic>{
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	{super.optionname="Choose band mark";}
	
	/**creates a scale bar picker with the given example scale bar*/
	public BandMarkExamplePicker(ArrowGraphic model) {
		super(model);
	}

	/**returns true if the object is an image panel in a type of figure that uses marks*/
	boolean isDesirableItem(Object o) {
		...
		if(o instanceof KnowsParentLayer) {
			...
			if(((KnowsParentLayer) o).getParentLayer() instanceof BandMarkLayer) {
				return true;
			} else return false;
	...
	public AbstractUndoableEdit applyProperties(Object item) {
		...
		if (item instanceof ArrowGraphic) {
				ArrowGraphic item2=(ArrowGraphic) item;
				AbstractUndoableEdit undo = item2.provideUndoForDialog();
				item2.copyAttributesFrom(getModelItem());
				item2.copyArrowAtributesFrom(getModelItem());
